...
| `$83:C7F8` | `Lufia2ActorPrimaryUpdate` | `draft` | Portable front-end reconstructs the known gates and `$7F:E3C6+slot` timer update through `$83:C83B`, with explicit continuations at `$83:C808`, `$83:C829` and `$83:C83C`. The supplied sampler-v3 snapshots matched flow, bus prefix, complete-return CPU state and WRAM with zero deviations. |
